...
export async function getFaturasMessage() {
  const response = await apiWhatsapp.get(
    'all-messages-in-chat/552180563748',
  );
...
export async function getAllMessagesContent() {
  try {
    ...
    Logger.warn('Houve um erro ao fazer chamada http para o wpp connect. Verifique as credenciais ou se a instância está ativa no momento')
    throw new HttpException({ error: "Não foi obter mensagens do whatsapp", describe: error}, HttpStatus.INTERNAL_SERVER_ERROR)
  }
}
...
export async function getLastMessage(){
  try {
    ...
    throw new HttpException({ error: "Erro enviar mensagem para a API do Whatsapp, verifique sua sessão do servidor", error1}, HttpStatus.INTERNAL_SERVER_ERROR)
  }
}
